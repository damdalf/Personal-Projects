...
Pokemon::Pokemon(int level) : level(level)
{
  ...
  for (i = 0; i < 6; i++) {
    ...
    if (i == 0) { // HP
      ...
      // initialize current hp as max hp
...
bool Pokemon::is_defeated() const
{
  return defeated;
}
...
int Pokemon::get_iv(int stat) const
{
  return IV[stat];
}
...
{
  return pokemon_stats[pokemon_species_index * 6 - 0].base_stat;
}
...
int Pokemon::get_move_id(int i) const
{
  if (i < 4 && move_index[i]) {
    return moves[move_index[i]].id;
  } else {
    return 0;
  }
}

int Pokemon::get_species_idx() const
{
...
void Pokemon::set_is_defeated(bool b)
{
  defeated = b;
}
